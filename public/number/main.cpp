...
int main()
{
	...
	FILE *fileptr=fopen("input.bmp","rb"); //The file needs to be named input exactly and is in BMP format
	if(!fileptr){
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for(r=0;r<R;r++){
        for(c=0;c<C;c++){
            unsigned int val = pic[r][c][0];
            val = val + pic[r][c][1];
            val = val + pic[r][c][2];
            val = val/3;
            if( val <= 80) { //make this pixel BLACK
                    pic[r][c][0] = 0;
                    pic[r][c][1] = 0;
                    pic[r][c][2] = 0;
                }
	...
	for(r=1;r<R-1;r++){
        for(c=1;c<C-1;c++){
            if(pic[r][c][0]==0){ //this is a black pixel?
                 if ((pic[r+1][c][0]==0) || (pic[r+1][c+1][0]==0) || (pic[r+1][c-1][0]==0)
                     || (pic[r][c+1][0]==0) || (pic[r-1][c+1][0]==0) || (pic[r-1][c][0]==0)
                     || (pic[r-1][c-1][0]==0) || (pic[r][c-1][0]==0)   ){

                 }  else {
                     pic[r][c][0]=255; pic[r][c][1]=255; pic[r][c][2]=255;
                 }
            }
        }
	}
	for(r=0;r<R;r++){
        for(c=0;c<C;c++){
            if(pic[r][c][0]==0){
                ...
                pic[r][c][0]=0; pic[r][c][1]=0; pic[r][c][2]=0;
            }
        }
	}
